...
;	main.c:25: ms = 65536 - ms;
	...
	clr	c
	subb	a,r4
	mov	_delay_ms_ms_10000_1,a
	clr	a
	subb	a,r5
	mov	(_delay_ms_ms_10000_1 + 1),a
	mov	a,#0x01
	subb	a,r6
	mov	(_delay_ms_ms_10000_1 + 2),a
	clr	a
	subb	a,r7
	mov	(_delay_ms_ms_10000_1 + 3),a
;	main.c:26: TH0 = ms / 256;
	...
	mov	dpl, _delay_ms_ms_10000_1
	mov	dph, (_delay_ms_ms_10000_1 + 1)
	mov	b, (_delay_ms_ms_10000_1 + 2)
	lcall	__divslong
	mov	r0, dpl
	mov	_TH0,r0
...
;	main.c:28: ms = 0;
	...
	mov	r7,#0x00
00103$:
;	main.c:30: TF0 = 0;  // Xóa cờ tràn Timer 0 Chuẩn bị cho lần đếm tiếp theo và Ngăn ngắt không mong muốn
;	assignBit
	clr	_TF0
...
;countled                  Allocated with name '_hieu_ung_2_countled_10001_13'
...
;	main.c:77: for (j = 0; j < 8; j++) {
	...
	mov	_j,a
	mov	(_j + 1),a
	mov	_hieu_ung_2_countled_10001_13,#0x08
	mov	(_hieu_ung_2_countled_10001_13 + 1),a
00113$:
...
;	main.c:80: for (k = 0; k < countled; k++) {
	...
	mov	_k,a
	mov	(_k + 1),a
	mov	a,_hieu_ung_2_countled_10001_13
	add	a,#0xff
	mov	r2,a
	mov	a,(_hieu_ung_2_countled_10001_13 + 1)
	addc	a,#0xff
	mov	r3,a
00111$:
	clr	c
	mov	a,_k
	subb	a,_hieu_ung_2_countled_10001_13
	mov	a,(_k + 1)
	xrl	a,#0x80
	mov	b,(_hieu_ung_2_countled_10001_13 + 1)
	xrl	b,#0x80
	subb	a,b
	jnc	00105$
...
;	main.c:93: countled--;  // Giảm vòng lặp dịch led
	dec	_hieu_ung_2_countled_10001_13
	mov	a,#0xff
	cjne	a,_hieu_ung_2_countled_10001_13,00171$
	dec	(_hieu_ung_2_countled_10001_13 + 1)
00171$:
;	main.c:77: for (j = 0; j < 8; j++) {
	inc	_j
	clr	a
...
00114$:
00103$:
;	main.c:145: EA = 1;  // Cho phép ngắt toàn cục
;	assignBit
	setb	_EA
;	main.c:146: }
	pop	psw
...
00114$:
00103$:
;	main.c:156: EA = 1;  // Cho phép ngắt toàn cục
;	assignBit
	setb	_EA
;	main.c:157: }
	pop	psw
...
;	main.c:162: while (1) {
00110$:
;	main.c:164: if (hieu_ung_hien_tai == 1) {
	mov	a,#0x01
...
00138$:
	sjmp	00107$
00139$:
;	main.c:165: hieu_ung_1();
	lcall	_hieu_ung_1
	sjmp	00110$
00107$:
;	main.c:166: } else if (hieu_ung_hien_tai == 2) {
	mov	a,#0x02
...
00140$:
	sjmp	00104$
00141$:
;	main.c:167: hieu_ung_2();
	lcall	_hieu_ung_2
	sjmp	00110$
00104$:
;	main.c:168: } else if (hieu_ung_hien_tai == 3) {
	mov	a,#0x03
